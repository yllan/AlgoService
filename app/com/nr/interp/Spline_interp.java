...
public class Spline_interp extends Base_interp {
  ...
  public Spline_interp(final double[] xv, final double[] yv, final double yp1, final double ypn) { // double yp1=1.e99, double ypn=1.e99
    super(xv,yv,2);
   ...
   * @param ypn
   */
  public void sety2(final double[] xv, final double[] yv, final double yp1, final double ypn){ // double yp1=1.e99, double ypn=1.e99
    int i,k;
    double p,qn,sig,un;
  ...
  public double rawinterp(final int jl, final double x) {
    ...
    double y,h,b,a;
    h=xx[khi]-xx[klo];
    if (h == 0.0) throw new IllegalArgumentException("Bad input to routine splint");
    a=(xx[khi]-x)/h;
    b=(x-xx[klo])/h;
