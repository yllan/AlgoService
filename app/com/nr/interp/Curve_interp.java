...
public class Curve_interp {
   ...
   * should not duplicate the first — the algorithm will connect them.)
   * 
   * @param ptsin
   * @param close
   */
  public Curve_interp(final double[][] ptsin, final boolean close){
    ...
    for (i=0;i<in;i++) {
      ...
      if (i>0) {
        ...
        if (s[i] == s[i-1]) throw new IllegalArgumentException("error in Curve_interp");
      }
    }
   ...
   * @return
   */
  private double fprime(final double[] x,final int off_x, final double[] y, final int off_y, final int pm) {
    double s1 = x[off_x+0]-x[off_x+pm*1], s2 = x[off_x+0]-x[off_x+pm*2], s3 = x[off_x+0]-x[off_x+pm*3],
      s12 = s1-s2, s13 = s1-s3, s23 = s2-s3;
    return -(s1*s2/(s13*s23*s3))*y[off_y+pm*3]+(s1*s3/(s12*s2*s23))*y[off_y+pm*2]
      -(s2*s3/(s1*s12*s13))*y[off_y+pm*1]+(1./s1+1./s2+1./s3)*y[off_y+0];
  }
