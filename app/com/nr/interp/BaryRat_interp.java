 ...
 * call interp for interpolated values. Note that no error estimate dy is calculated.
 * 
...
public class BaryRat_interp extends Base_interp {
  ...
  public BaryRat_interp(final double[] xv, final double[] yv, final int dd) {
    ...
    if (n<=d) throw new IllegalArgumentException("d too large for number of points in BaryRat_interp");
    for (int k=0;k<n;k++) {
      int imin=max(k-d,0);
  ...
  /**
   * Use equation (3.4.9) to compute the barycentric rational interpolant. Note
   * that jl is not used since the approximation is global; it is included only
   ...
   * override interp to simply call rawinterp directly with a dummy value of jl.
   */
