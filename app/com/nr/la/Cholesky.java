...
public class Cholesky {
  ...
  /**
   * Given a positive-definite symmetric matrix a[0..n-1][0..n-1], construct and
   * store its Cholesky decomposition, A = L* Lt .
   * 
   * @param a
   */
  ...
  /**
   * Solve the set of n linear equations A*x = b, where a is a positive-definite
   * symmetric matrix whose Cholesky decomposition has been stored. b[0..n-1] is
   * input as the right-hand side vector. The solution vector is returned in
   * x[0..n-1].
   * 
   * @param b
   * @param x
  ...
  public void solve(final double[] b, final double[] x) {
    int i,k;
    double sum;
    if (b.length != n || x.length != n) throw new IllegalArgumentException("bad lengths in Cholesky");
    for (i=0;i<n;i++) {
