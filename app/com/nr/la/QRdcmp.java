 ...
 * useful, the so-called QR decomposition, A=Q*R Here R is upper triangular,
 * while Q is orthogonal, that is, QT*Q = 1
 * 
...
public class QRdcmp {
  ...
  /**
   * Solve the triangular set of n linear equations R*x = b. b[0..n-1] is input
   * as the right-hand side vector, and x[0..n-1] is returned as the solution
   * vector
   * 
   * @param b
   * @param x
  ...
  public void rsolve(final double[] b, final double[] x) {
    int i,j;
    double sum;
    if (sing) throw new IllegalArgumentException("attempting solve in a singular QR");
    for (i=n-1;i>=0;i--) {
      sum=b[i];
   ...
   * decomposition of the matrix Q*(R + u(+)v). Input quantities are u[0..n-1],
   * and v[0..n-1].
   * 
   * @param u
   * @param v
