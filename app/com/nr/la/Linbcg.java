...
public abstract class Linbcg {
  ...
  public void solve(final double[] b, final double[] x, final int itol, final double tol,
      final int itmax, final intW iter, final doubleW err) {
    ...
    if (itol == 1) {
      ...
      asolve(r,z,0);
      znrm=snrm(z,itol);
    } else throw new IllegalArgumentException("illegal itol in linbcg");
    while (iter.val < itmax) {
      ...
      if (itol == 1)
        ...
        xnrm=snrm(x,itol);
        if (err.val <= 0.5*xnrm) err.val /= xnrm;
        else {
          err.val=znrm/bnrm;
          continue;
        }
      }
      if (err.val <= tol) break;
    }
  }
  
  /**
   * Compute one of two norms for a vector sx[0..n-1], as signaled by itol. Used
   * by solve.
   * 
   * @param sx
   * @param itol
