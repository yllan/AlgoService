 ...
 * 
 * Object for solving linear equations A*x = b for a band-diagonal matrix A,  using LU decomposition
 * 
...
public class Bandec {
  ...
  /*  Constructor. Given an nXn band-diagonal matrix A with m1 subdiagonal rows and m2 superdiagonal
   rows, compactly stored in the array a[0..n-1][0..m1+m2] as described in the comment for the routine bandmul, 
   an LU decomposition of a rowwise permutation of A is constructed. The upper and lower triangular matrices
   are stored in au and al, respectively. The stored vector indx[0..n-1] records the row permutation 
   effected by the partial pivoting; d is +-1 depending on whether the number of row interchanges was even or odd, 
   respectively
  ...
  public Bandec(final double[][] a, final int mm1, final int mm2) {
    ...
    for (k=0;k<n;k++) {
      ...
      if (i != k) {
        ...
        for (j=0;j<mm;j++) {
          // SWAP(au[k][j],au[i][j]);
          double swap = au[k][j]; au[k][j] = au[i][j]; au[i][j] = swap;
        }
      }
      for (i=k+1;i<l;i++) {
        dum=au[i][0]/au[k][0];
  ...
  /**
   * Given a right-hand side vector b[0..n-1], solves the band-diagonal linear equations A*x = b.
